...
class Scenario(BaseScenario):
    ...
    def reward_all_in_once(self, agent, world):
        ...
        v = [l.state.p_pos for l in world.food]
        u = u - np.mean(u, 0)
        v = v - np.mean(v, 0)
        reward_n -= max(directed_hausdorff(u, v)[0], directed_hausdorff(v, u)[0])
    ...
    def observation(self, agent, world):
        ...
        result = np.concatenate([agent.state.p_vel] + [[0,0]] + [np.array([agent.live])] + entity_pos + other_pos + other_vel + other_live)
        return result
